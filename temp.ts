...
export const GET: RequestHandler = async () => {
	...
	const schedule: Record<
		string,
		{ waktu: string; mata_kuliah: MatkulDetil; dosen: string[]; ruang: string; jenis: string }[]
	> = {};
	...
	table.find('tr').each((rowIndex, row) => {
		...
		const rawSpan = $(row).find('td').attr('rowspan');
		...
		if (rawSpan) {
			...
			for (let i = 0; i < Number(rawSpan); i++) {
				date.setMinutes(date.getMinutes() + 50); // Tambah 50 menit

				// **Cek aturan khusus loncatan waktu**
		...
		cells.each((cellIndex, cell) => {
			...
			console.log(`Cell Index: ${cellIndex}`); // Debugging log
